  ...
  return (
    <div className="bg-white dark:bg-black/20 rounded-xl p-6 shadow-sm">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-sm font-medium text-black dark:text-primary/80">{label}</h3>
        <span className={`p-1.5 rounded-lg ${getIconBgColor()}`}>
          <i className={`ri-${icon}`}></i>
        </span>
      </div>
      <div className="flex items-baseline space-x-1">
        <span className="text-2xl font-bold text-black dark:text-primary">{value}</span>
        {change && (
          <span className={`text-xs font-medium ${getChangeColor()}`}>{change}</span>
        )}
      </div>
      {subtitle && (
        <div className="mt-1 text-xs text-black/70 dark:text-primary/60">
          <span>{subtitle}</span>
        </div>
      )}
    </div>
